            ...
            reader.Setup(x => x.GetValues(It.IsAny<object[]>()))
                .Callback<object[]>(
                    (values) =>
                    {
                        ...
                        values[2] = tstDataSet[count]["start_date"];
                        values[3] = tstDataSet[count]["is_primary"];
                        values[4] = tstDataSet[count]["enum_session_id"];
                        values[5] = tstDataSet[count]["organization_id"];
